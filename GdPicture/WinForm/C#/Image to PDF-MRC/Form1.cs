        ...
        public void Button3_Click(Object sender, EventArgs e)
        {
            try
            {
                ...
                using (GdPictureImaging gdpictureImaging = new GdPictureImaging())
                {
                    ...
                    {
                        PdfConformance conformance = GetSelectedConformance();
                        gdpicturePDF.NewPDF(conformance);
                        gdpicturePDF.SetMRCPreserveSmoothing(chkPreserveSmoothing.Checked);
                        gdpicturePDF.SetMRCImageBackgroundResolution((float)backgroundResolution.Value);
                        gdpicturePDF.SetCompressionForBitonalImage(PdfCompression.PdfCompressionJBIG2);

                        if (float.Parse(gdpicturePDF.GetVersion()) > 1.4)
                        {//jpeg 2000 compression is supported only starting PDF 1.5.
                            gdpicturePDF.SetCompressionForColorImage(PdfCompression.PdfCompressionJPEG2000);
                            gdpicturePDF.SetJpeg2000Quality(-(int)quality.Value);//using negative value permits to specify a quality factor vs a compression rate. (This should be documented soon).
                        }
                        else
                        {
                            gdpicturePDF.SetCompressionForColorImage(PdfCompression.PdfCompressionJPEG);
                            gdpicturePDF.SetJpegQuality((int)quality.Value);
                        }

                        //gdpicturePDF.SetMRCImageBackgroundResolution

                        for (int pageNo = 1; pageNo <= pageCount; pageNo++)
                        {
                            gdpictureImaging.SelectPage(imageID, pageNo);
                            gdpicturePDF.AddImageFromGdPictureImage(imageID, PdfAdvancedImageCompression.PdfAdvancedImageCompressionMRC);
